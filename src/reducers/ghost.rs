    ...
    state
        ...
        .fold((state.rand, vec![]), |(rand, ghosts), ghost| {
            ...
            let all_dirs: Vec<Vec2d<i32>> = [[1, 0], [0, 1], [-1, 0], [0, -1]]
                ...
                .map(|d| (*d).into())
                .collect();
            let available_dirs: Vec<Vec2d<i32>> = all_dirs.into_iter().fold(vec![], |acc, dir| {
            ...
            let next_dir = if on_square {
                available_dirs[idx]
            } else {
                ghost.vel
            };
            let new_vel =
                if on_square && map_hit(&state.map, cur_map_pos + desired_vel) == MapCell::Wall {
                    next_dir
                } else {
                    desired_vel
                };
